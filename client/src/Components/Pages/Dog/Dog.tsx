...
interface Props {
	...
	>;
	dislikedDogs: Dog[];
	setDislikedDogs: React.Dispatch<
		React.SetStateAction<
			{
				message: string;
				status: string;
...
function Dog({ likedDogs, setLikedDogs, dislikedDogs, setDislikedDogs }: Props) {
	...
	const dogsArr = Object.entries(dogs)
	...
	useEffect(() => {
		...
		if (isDogLoading) {
			...
			const likeHandler = (dog: { message: string; status: string }) => {
			...
			const dislikeHandler = (dog: { message: string; status: string }) => {
	...
	return (
		<Card style={{ width: "600px", height: "510px" }}>
			{dogsArr.map(dog =>
				<>
							<CardMedia component='img' alt='doge' height='400' image={dogsArr[0][1]} key={dog[0][1]} />
			<CardActions>
